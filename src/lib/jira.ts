...
/**
 * Fetches tickets from JIRA for a given label
 */
export async function fetchTicketsByLabel(label: string): Promise<JiraTicket[]> {
  if (!JIRA_BASE_URL || !JIRA_API_TOKEN) {
    throw new Error('JIRA_BASE_URL or JIRA_API_TOKEN is not defined in environment variables');
  }

  const client = getJiraClient();

  const response = await client.get(
    `${JIRA_BASE_URL}/rest/api/2/search?jql=labels=${label}&maxResults=1000`,
    {
      headers: {
 ...
 */
export async function fetchTicketsByLabels(labels: string[]): Promise<JiraTicket[]> {
...
export function isTicketCompleted(ticket: JiraTicket): boolean {
  return ticket.fields.resolution !== null &&
 ...
 */
export function processRowData(rowConfig: RowConfig, tickets: JiraTicket[]): RowData {
  ...
  const completedStoryPoints = completedTickets.reduce((sum, ticket) => {
    return sum + (ticket.fields[STORY_POINTS_FIELD as keyof typeof ticket.fields] as number || 0);
  }, 0);
